...
fn main() {
    ...
    for nums in test_cases.iter() {
        println!(
            "\x1b[36m{:?} => {}\x1b[0m",
            nums,
            Solution::minimum_deletions(nums.clone())
        );
    }
}
...
impl Solution {
    pub fn minimum_deletions(nums: Vec<i32>) -> i32 {
        // Naive way of getting the min/max at index, but we also have to be careful about duplicates and determining which way to go round.
        // After submission note: we do not need to be careful about duplicates...
        let mut mins = Vec::<(usize, i32)>::new(); // list of tuples which should hold the min value and each of its indices
        let mut maxs = Vec::<(usize, i32)>::new(); // list of tuples which should hold the max value and each of its indices
        for (i, &item) in nums.iter().enumerate() {
            ...
            if item < mins[0].1 {
                mins = Vec::<(usize, i32)>::new();
                mins.push((i, item));
            }
            if item > maxs[0].1 {
                maxs = Vec::<(usize, i32)>::new();
                maxs.push((i, item));
            }
        }
        ...
        // We should, in theory, have a list of all the mins and maxes, and we should also be able to pop(0) and pop(-1) on both (assuming duplicates).

        // Case 1: max = min
        if maxs[0].1 == mins[0].1 {
            debug!("Case 1: Exiting quickly with 1!");
            return 1;
        }

        // Case 2: everything else (dupes, no dupes, etc.)
        let first_min_idx = mins[0].0 as i32;
        let last_min_idx = mins[mins.len() - 1].0 as i32;
        let first_max_idx = maxs[0].0 as i32;
        let last_max_idx = maxs[maxs.len() - 1].0 as i32;
        let len = nums.len() as i32;

        debug!("Case 2:");
        debug!(
            "first_min_idx = {}, last_min_idx = {}, first_max_idx = {}, last_max_idx = {}",
            first_min_idx, last_min_idx, first_max_idx, last_max_idx
        );

        // Case 2.a: No dupes
        if first_min_idx == last_min_idx && first_max_idx == last_max_idx {
            // mX or Xm --> [..., min, ..., max, ...] or [..., max, ..., min, ...]
            debug!("Case 2.a: No dupes!");

            // Normalize these by POSITION rather than min/max:
            //
            // [... left ... right ...]
            let (left_idx, right_idx, left_label, right_label) = if first_min_idx < first_max_idx {
                (first_min_idx, first_max_idx, 'm', 'X')
            } else {
                (first_max_idx, first_min_idx, 'X', 'm')
            };

            debug!(
                "left = {} @ {}, right = {} @ {}",
                left_label, left_idx, right_label, right_idx
            );

            // Valid strategies:
            //
            // [.........R]........     remove both from left
            // ........[L.........]     remove both from right
            // [L.......].....[R..]     remove from both ends
            let from_left = right_idx + 1;
            let from_right = len - left_idx;
            let from_both_ends = left_idx + 1 + len - right_idx;

            debug!("from_left      = {}", from_left);
            debug!("from_right     = {}", from_right);
            debug!("from_both_ends = {}", from_both_ends);

            if from_left <= from_right && from_left <= from_both_ends {
                debug!("Case 2.a.1: Grab both from left through {}", right_label);
                return from_left;
            }

            if from_right <= from_left && from_right <= from_both_ends {
                debug!("Case 2.a.2: Grab both from right through {}", left_label);
                return from_right;
            }

            debug!(
                "Case 2.a.3: Grab from both ends ({}...{})",
                left_label, right_label
            );
            return from_both_ends;
        }

        // Case 2.b: There are dupes.
        //
        // Possibilities:
        // [..., min1, ..., min2, ..., max1, ..., max2, ...] mmXX - 1
        // [..., min1, ..., max1, ..., min2, ..., max2, ...] mXmX - 2
        // [..., min1, ..., max1, ..., max2, ..., min2, ...] mXXm - 3
        // [..., max1, ..., min1, ..., min2, ..., max2, ...] XmmX - 3'
        // [..., max1, ..., min1, ..., max2, ..., min2, ...] XmXm - 2'
        // [..., max1, ..., max2, ..., min1, ..., min2, ...] XXmm - 1'

        // Case 2.b.1: mmXX or XXmm
        if last_min_idx < first_max_idx || last_max_idx < first_min_idx {
            debug!("Case 2.b.1: mmXX or XXmm");

            // Normalize the groups by POSITION:
            //
            // [... left1 ... left2 ... right1 ... right2 ...]
            let (left_first, left_last, right_first, right_last, left_label, right_label) =
                if last_min_idx < first_max_idx {
                    (
                        first_min_idx,
                        last_min_idx,
                        first_max_idx,
            ...
            // Valid strategies:
            //
            // mmX.  / XXm.  --> through first element of right group from left
            // .mXX  / .Xmm  --> through last element of left group from right
            // m..X  / X..m  --> take the outside members from both ends
            let from_left = right_first + 1;
            let from_right = len - left_last;
            let from_both_ends = left_first + 1 + len - right_last;
            ...
            if from_right <= from_left && from_right <= from_both_ends {
                debug!(
                    "Case 2.b.1.b: grab from right (.{}{}{})",
                    left_label, right_label, right_label
                );
                return from_right;
            }

            debug!(
                "Case 2.b.1.c: grab from both ends ({}..{})",
                left_label, right_label
            );
            return from_both_ends;
        }

        // Case 2.b.2: mXmX or XmXm
        if (first_min_idx < first_max_idx
            && first_max_idx < last_min_idx
            && last_min_idx < last_max_idx)
            || (first_max_idx < first_min_idx
                && first_min_idx < last_max_idx
                && last_max_idx < last_min_idx)
        {
            debug!("Case 2.b.2: mXmX or XmXm");

            // Normalize by POSITION:
            //
            // [... left1 ... right1 ... left2 ... right2 ...]
            let (left_first, right_first, left_last, right_last, left_label, right_label) =
                if first_min_idx < first_max_idx {
                    (
                        first_min_idx,
                        first_max_idx,
            ...
            // Valid strategies:
            //
            // mX.. / Xm..   --> grab through right_first from left
            // ..mX / ..Xm   --> grab through left_last from right
            // m..X / X..m   --> grab the outside pair
            let from_left = right_first + 1;
            let from_right = len - left_last;
            let from_both_ends = left_first + 1 + len - right_last;
            ...
            debug!(
                "Case 2.b.2.c: grab from both ends ({}..{})",
                left_label, right_label
            );
            return from_both_ends;
        }

        // Case 2.b.3: mXXm or XmmX
        if (first_min_idx < first_max_idx && last_max_idx < last_min_idx)
            || (first_max_idx < first_min_idx && last_min_idx < last_max_idx)
        {
            debug!("Case 2.b.3: mXXm or XmmX");

            // Normalize by POSITION:
            //
            // [... outer_left ... inner_left ... inner_right ... outer_right ...]
            let (outer_left, inner_left, inner_right, outer_right, outer_label, inner_label) =
                if first_min_idx < first_max_idx {
                    (
                        first_min_idx,
                        first_max_idx,
            ...
            // Only two useful strategies here:
            //
            // mX.. / Xm..   --> grab through inner_left from left
            // ..Xm / ..mX   --> grab through inner_right from right
            let from_left = inner_left + 1;
            let from_right = len - inner_right;
